...
Đây là status của **workflow page** (chính bản thân SOP/template) — không phải của từng case chạy qua workflow đó. Status này nằm ở field `Definition Status` trong metadata header (Tầng 1a) và được mirror sang label `status:*` để filter.

```
DRAFT → IN_REVIEW → ACTIVE → DEPRECATED → ARCHIVED
            │                     │
            └──── (cần sửa) ──────┘ → quay lại DRAFT
ACTIVE ──(revision lớn)──► IN_REVIEW
```

| Status      | Ý nghĩa                                                  | Agent execute? | Bước tiếp theo                          |
|-------------|----------------------------------------------------------|----------------|-----------------------------------------|
| `DRAFT`     | Đang viết nội dung, chưa hoàn chỉnh                      | ❌             | → IN_REVIEW                             |
| `IN_REVIEW` | Nội dung xong, chờ owner/stakeholder duyệt              | ❌             | → ACTIVE (approve) / → DRAFT (cần sửa)  |
| `ACTIVE`    | Đã duyệt, là version chính thức — agent được phép chạy  | ✅             | → DEPRECATED / → IN_REVIEW (sửa lớn)    |
| `DEPRECATED`| Có version mới hơn / sắp ngừng dùng                     | ⚠️ chỉ khi user xác nhận | → ARCHIVED / → ACTIVE (revive) |
| `ARCHIVED`  | Ngừng hẳn, chỉ giữ tham khảo lịch sử                    | ❌             | (terminal)                              |

**Quy tắc execute:** Agent chỉ tự động execute workflow có `Definition Status = ACTIVE`. Với `DEPRECATED`, agent cảnh báo "workflow này đã có bản thay thế" và chỉ chạy nếu user xác nhận. Các status còn lại (`DRAFT`, `IN_REVIEW`, `ARCHIVED`): agent có thể search/hiển thị thông tin nhưng **không** execute. `ARCHIVED` mặc định bị ẩn khỏi Discovery (Tầng 2).

> Tên status ở đây cố tình khác với instance lifecycle (`SUBMITTED / UNDER REVIEW / APPROVED / DONE`...) để hai vòng đời không bị nhầm lẫn: đây là vòng đời của **template**, còn instance lifecycle là vòng đời của **từng case**.
