...
class GraphState(TypedDict, total=False):
    ...
    """Up to 3 proactive follow-up questions generated by the suggest node."""

    # ── Workflow execution (set by discover_workflow / execute_workflow) ───────
    workflow_mode: bool
    """True once the run is on the workflow-execution path (set by discovery)."""

    workflow_name: str | None
    """Name of the discovered workflow page (or None when none matched)."""

    workflow_page_id: str | None
    """Confluence page id of the chosen workflow; None ⇒ graceful no-match reply."""

    workflow_candidates: list[dict]
    """Top alternative workflows ``{name, page_id, score}`` (transparency/logging)."""

    jira_source: Optional[str]
    """``existing-ticket`` | ``auto-create`` — parsed from the workflow definition."""

    jira_parent_key: Optional[str]
    """Jira ticket key extracted from the request (for ``existing-ticket`` actions)."""

    workflow_discovery_note: Optional[str]
    """Short human note when discovery found nothing (executor turns it into a reply)."""
