...
def test_encode_query_shape_and_normalized(embedder: Embedder) -> None:
    ...
    embedder._http = MagicMock()
    embedder._http.post.return_value = _mock_response([raw])
    ...
    payload = embedder._http.post.call_args.kwargs["json"]
    assert payload["input"] == ["hello world"]
    assert payload["encoding_format"] == "dense"
    assert payload["model"] == "baai/bge-m3"


def test_encode_passages_shape_and_batch(embedder: Embedder) -> None:
    vecs = [[1.0] + [0.0] * (_BGE_M3_DIM - 1), [0.0, 1.0] + [0.0] * (_BGE_M3_DIM - 2)]
    embedder._http = MagicMock()
    embedder._http.post.return_value = _mock_response(vecs)
...
def test_encode_passages_empty_returns_zero_rows(embedder: Embedder) -> None:
    ...
    assert matrix.dtype == np.float32
    embedder._http.post.assert_not_called()
